...
TIMESTAMP=$(date +%s)
OUTPUT_DIR="/tmp/pragmatic-tests/${TIMESTAMP}/explicit-skill-requests/multiturn"
mkdir -p "$OUTPUT_DIR"
...
cat > "$PROJECT_DIR/docs/pragmatic/plans/auth-system.md" << 'EOF'
...
claude -p "Good analysis. I've already written the plan to docs/pragmatic/plans/auth-system.md. Now I'm ready to implement. What are my options for execution?" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
